...
function generarPartidos(req, res) {
    var idLiga = req.params.nombreL;
    ...
    Equipo.findById(idEquipo).populate('liga', 'nombre').exec((err, listadeequipos) => {
            ...
            partidos.save(),
                ...
                modelResultado.save((err, resultado) => {
                    if (err) {
                        ...
                        return res.status(500).send({mensaje: "Resultado almacenado"});
                    }
        })
        }
        console.log(listadeequipos)
    })
